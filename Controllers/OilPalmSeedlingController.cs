...
namespace Ade_Farming.Controllers
{
    ...
    public class OilPalmSeedlingController : Controller
    {
        private readonly UserManager<ApplicationUser> _userManager;
        private readonly IMongoCollection<OilPalmSeedlingPost> _posts;
        private readonly ILogger<OilPalmSeedlingController> _logger;

        public OilPalmSeedlingController(
            UserManager<ApplicationUser> userManager,
            IMongoDatabase database,
            ILogger<OilPalmSeedlingController> logger)
        {
            _userManager = userManager;
            _posts = database.GetCollection<OilPalmSeedlingPost>("OilPalmSeedlingPosts");
            _logger = logger;
        }
        ...
        public async Task<IActionResult> Create(OilPalmSeedlingPostViewModel model)
        {
            ...
            try
            {
                ...
                var post = new OilPalmSeedlingPost
                {
                    Title = model.Title,
            ...
            {
                _logger.LogError(ex, "Error creating OilPalmSeedlingPost");
        ...
        public async Task<IActionResult> MyPosts()
        {
            try
            {
                ...
                var filter = Builders<OilPalmSeedlingPost>.Filter.Eq(p => p.SellerId, user.Id.ToString());
            ...
            catch (Exception ex)
            {
                ...
                return View(Enumerable.Empty<OilPalmSeedlingPost>());
            }
        }
        ...
        public async Task<IActionResult> Edit(string id)
        {
            ...
            {
                var user = await _userManager.GetUserAsync(User);
                var filter = Builders<OilPalmSeedlingPost>.Filter.Eq(p => p.Id, id) &
                             Builders<OilPalmSeedlingPost>.Filter.Eq(p => p.SellerId, user.Id.ToString());

                var post = await _posts.Find(filter).FirstOrDefaultAsync();
                if (post == null) return NotFound();

                var model = new OilPalmSeedlingPostViewModel
                {
                    Id = post.Id,
                    Title = post.Title,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(string id, OilPalmSeedlingPostViewModel model)
        {
            ...
            try
            {
                ...
                var update = Builders<OilPalmSeedlingPost>.Update
            ...
            {
                _logger.LogError(ex, "Error updating OilPalmSeedlingPost Id={Id}", id);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            {
                _logger.LogError(ex, "Error deleting OilPalmSeedlingPost Id={Id}", id);
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                var posts = await _posts.Find(Builders<OilPalmSeedlingPost>.Filter.Empty).ToListAsync();
                return View(posts);
            }
            catch (Exception ex)
            {
